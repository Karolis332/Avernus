# Campaign State Tracker

## Party
- **Level:** 11
- **Size:** 3 players (was 4 — Xebeche and Rebecca had earlier arcs)
- **Current PCs:** Drenwal, Aurora, Asimov
- **Companions:** Whiskerbright — Aurora's familiar, currently in imp form (Avernus adaptation)

## Current Location
Fort Knucklebone — just completed Mad Maggie's Dream Sequence

## What the Party Knows
- The **Fallen Three** (Olanthius, Haruman, Bel) each know part of the path to the Bleeding Citadel
- **Intermediaries** (Mordenkainen, Red Ruth, Mephistopheles) can locate the Fallen Three — but require **blood pay**
- The importance of the **blood cost** — powerful creature blood is needed
- **Zariel's Sword** is in the Bleeding Citadel and is the key to freeing Elturel
- **Kostchtchie's Maw** exists (freeing the demon lord can distract Zariel)
- The **Solar's Helmet** exists
- **Drenwal's brother Dumal** and another Bhaalspawn are in Hell
- Drenwal **severed his amulet connection** — they cannot be tracked (but also cannot track Dumal)
- **Haruman has NOT been killed yet** in this campaign

## What Has Happened
- Completed Chapters 1-2 (Baldur's Gate → Elturel → Avernus)
- Arrived at Fort Knucklebone
- Met Mad Maggie, completed the Dream Quest (5 phases with saves)
- Dream revealed the Fallen Three and hinted at Intermediaries
- Party has the Shield of the Hidden Lord (Gargauth)

## Active Plot Threads
1. **Main Quest:** Find the Bleeding Citadel → retrieve Zariel's Sword → free Elturel
2. **Drenwal's Arc:** Bhaal vs Gargauth tug-of-war. Brother Dumal hunting him. Bhaalspawn siblings (Maelys, Valruk, Dumal) converging
3. **Aurora's Arc:** Moonkite imprisoned in Avernus by Zariel's forces. Wand of Celestial Warding pulses near Moonkite artifacts. She feels pulled deeper into Avernus
4. **Gargauth's Manipulation:** Tempting Drenwal with power; previously posed as Helm to Xebeche
5. **Time Pressure:** Elturel continues sinking into the River Styx

## Session Log

### Session 1 — 2026-02-15
**Location:** Fort Knucklebone area
**Events:**
- Party encountered the arcane scarecrows (ward-markers in the charred forest)
- Asimov threw bread at the scarecrows — they started screaming for a prolonged period
- Aurora sent Whiskerbright forward — he triggered/exploded one of the scarecrows
- Aurora shot and destroyed 2 more scarecrows — massive noise
- The commotion attracted **Bloodbath the Drinker of Gore** and his forces
- Bloodbath arrived on his bike (modified Devil's Ride / Meatgrinder)
- Party dealt 191 total damage: 130 to the bike (destroyed), 61 to Bloodbath
- Bike explosion knocked Bloodbath unconscious on impact
- Bloodbath's HP buffed to 350 for this encounter (289 HP remaining)
- Asimov's genie **Rasheem** attempted to consume the unconscious Bloodbath
- Bloodbath woke up at the last moment before being consumed
**Consequences:** Bloodbath encounter triggered by scarecrow noise. Bike destroyed. Bloodbath regained consciousness — 289 HP, all 3 Legendary Resistances intact, and now he's angry.
**Items gained/lost:** —
**NPC reactions:** —
